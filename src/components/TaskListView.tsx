...
export function TaskListView({ timer }: { timer: Timer }) {
  ...
  const handleEstimateSave = (taskId: string) => {
    const val = parseInt(editEstimateValue, 10);
    if (!isNaN(val) && val > 0) {
      timer.updateEstimate(taskId, val);
    }
                    ...
                    {editingEstimateId === task.id ? (
                      <span className="inline-edit">
                        <input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === 'Enter') handleEstimateSave(task.id);
                            if (e.key === 'Escape') setEditingEstimateId(null);
                          }}
                          onBlur={() => handleEstimateSave(task.id)}
                      ...
                      <span
                        ...
                        onClick={() => {
                          if (!task.isCompleted) {
                            ...
                            setEditEstimateValue(String(task.estimatedMinutes));
                          }
                        }}
